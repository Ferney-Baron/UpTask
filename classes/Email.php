...
class Email {
    ...
    public function enviarRecuperacion() {
        ...
        $mail->setFrom('admin@uptask.com');
        $mail->addAddress('admin@uptask.com', 'uptask.com');
        $mail->Subject = 'Reestablace tu Password';

        $mail->isHTML(true);
        $mail->CharSet = 'utf-8';

        $enlace = "http://localhost:3000/reestablecer?token=" . $this->token ;

        $contenido = '<html>';
        $contenido .= "<p><strong>Hola " . $this->nombre . " </strong>Para Recuperar tu Cuenta, Ingresa al Siguiente Enlace</p>";
        $contenido .= "<p>Presiona Aquí: <a href='$enlace'>Activar Cuenta</a></p>";
        $contenido .= "Si no has solicitado cambiar la clave, ignora este mensaje";
        $contenido .= '<html>';
